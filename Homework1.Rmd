...
Alright, now let's see if deprivation indeed impacted TMA score. We'll run a plot similar to the one above, except by substituting highest level of education with deprivation. That means we'll be parsing out differences in score by gender and final result designation as well. 
...
ggplot(ses_info_assess, aes(x = imd_band, y = score, fill = gender)) +
  ...
  coord_flip() +
  facet_wrap(~final_result, ncol = 1) +
  labs(x = "Deprivation", y = "Average Score", title = "Average TMA Scores", subtitle = "Results displayed by deprivation, gender, and final result designation") +
  scale_fill_discrete(name = "Gender")
```
...
This density plot is somewhat revealing. Interestingly, it looks like a greater proportion of 0-10% IMD band students had lower TMA scores than 90-100% IMD band students. In other words, the underlying density distribution for 90-100% IMD band students was more negatively skewed than it was for 0-10% IMD band students.
